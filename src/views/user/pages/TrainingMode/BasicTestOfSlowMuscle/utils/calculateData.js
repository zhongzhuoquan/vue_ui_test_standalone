 ...
 * @LastEditTime: 2020-12-19 10:14:14
 * @Description :
 */
export function calculateData(standardArray, testArray, time) {
  ...
  for (let i = 59 + time * 10; i <= 79 + time * 10; i++) {
    if (
      standardArray[i] - 1 < testArray[i] &&
      testArray[i] < standardArray[i] + 1
    ) {
  ...
  score_relax = parseInt((score_relax / scoreArray_relax.length).toFixed(2))
  //放松部分
  ...
  score_shrink = parseInt((score_shrink / scoreArray_shrink.length).toFixed(2))
  //收缩部分

  //保持部分
  for (let i = 40; i <= 39 + time * 10; i++) {
    if (
      standardArray[i] - 1 < testArray[i] &&
      testArray[i] < standardArray[i] + 1
    ) {
  ...
  score_keep = parseInt((score_keep / scoreArray_keep.length).toFixed(2))

  //保持部分

  //逐渐放松
  for (let i = 40 + time * 10; i <= 59 + time * 10; i++) {
    if (
      standardArray[i] - 1 < testArray[i] &&
      testArray[i] < standardArray[i] + 1
    ) {
  ...
  score_loosen = parseInt((score_loosen / scoreArray_loosen.length).toFixed(2))
  //逐渐放松
  data = [
    score_relax,
    score_keep,
